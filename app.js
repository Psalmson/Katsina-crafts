...
function StatusBadge({ status }) {
  ...
  return <span style={{padding:'4px 12px',borderRadius:20,fontSize:11,fontWeight:600,background:s.bg,color:s.color,letterSpacing:0.5,whiteSpace:'nowrap'}}>{s.label}</span>;
}
...
function AuthModal({ mode, onClose, onArtisanLogin, onAdminLogin, showToast }) {
  ...
  const handleArtisanLogin = async () => {
    if (!login.email || !login.password) return showToast('Please fill in all fields', 'error');
    setLoading(true);
    const hash = await simpleHash(login.password);
  ...
  const handleSignup = async () => {
    ...
    const { error } = await db.from('artisans').insert({
          ...
          <div style={{display:'flex'}}>
            {[['login','Log In'],['signup','Join as Artisan']].map(([key,label]) => (
              <button key={key} onClick={() => setTab(key)} style={{
                ...
                borderBottom: tab===key ? '2px solid #C8913A' : '2px solid transparent',
                color: tab===key ? '#8B4513' : '#B8956A',
          ...
          {tab === 'login' && (
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              ...
              <div style={{display:'flex',background:'#F5EDE0',borderRadius:10,padding:4,gap:4}}>
                {[['artisan','Artisan'],['admin','Admin']].map(([type,label]) => (
                  <button key={type} onClick={() => setLoginType(type)} style={{
                    ...
                    color: loginType===type ? '#8B4513' : '#B8956A',
              ...
              <button className="btn btn-primary btn-full" style={{padding:'14px',marginTop:4}}
                onClick={loginType==='admin' ? handleAdminLogin : handleArtisanLogin}
                disabled={loading}>
                {loading ? <Spinner size={18} color="#fff" /> : `Log In as ${loginType==='admin'?'Admin':'Artisan'}`}
              </button>
              <p style={{textAlign:'center',fontSize:13,color:'#B8956A',margin:0}}>
                Want to sell?{' '}
                <button onClick={() => setTab('signup')} style={{background:'none',border:'none',color:'#8B4513',fontWeight:600,cursor:'pointer',fontSize:13,fontFamily:'inherit'}}>
                  Apply as an artisan →
                </button>
              </p>
            </div>
    ...
    <div style={{paddingTop:76,minHeight:'100vh',background:'var(--cream)'}}>
      <div style={{maxWidth:920,margin:'0 auto',padding:'40px 36px 80px'}}>
        ...
        <div style={{marginBottom:28}}>
          <h1 style={{fontFamily:'var(--font-display)',fontSize:'clamp(24px,4vw,36px)',fontWeight:600,color:'var(--earth)',letterSpacing:'-0.5px'}}>{artisan.shop_name}</h1>
      ...
      <div style={{maxWidth:1020,margin:'0 auto',padding:'40px 36px 80px'}}>
        ...
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:28,flexWrap:'wrap',gap:16}}>
          ...
          <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
            {pending_a.length>0 && <span style={{background:'#FEF3C7',color:'#92400E',padding:'6px 14px',borderRadius:20,fontSize:12,fontWeight:600}}>{pending_a.length} artisan{pending_a.length>1?'s':''} pending</span>}
            {pending_p.length>0 && <span style={{background:'#DBEAFE',color:'#1E40AF',padding:'6px 14px',borderRadius:20,fontSize:12,fontWeight:600}}>{pending_p.length} product{pending_p.length>1?'s':''} pending</span>}
          </div>
        </div>
...
function Navbar({ route, navigate, cartCount, artisan, adminUser, onOpenAuth, onLogout }) {
